export default {
  ...
  stringSeparators: /(?<!(?:\b\w)|(?:\b\w\w))(?<=\w)[.;]|(?:(?:,|(?<=[)}\]]))\s?[Aa]nd)(?=\s?\S)/g, // character
  leadingTrailingPunctAndWhite: /^\W*|\s*[,.]\s*$/g,
  whiteBeforePunct: /(?:\s)([^\\/[{()}\]])(?=\s)/g,
  slashOrOpening: /((?<=[[:alpha]])[\\/[{(])/g, // character
  slashOrClosing: /([\\/)}\]](?=[[:alpha]]))/g, // character
  slashDashOrOpening: /((?<=[[:alpha]])[\\/[{(-])/g, // character
  slashDashOrClosing: /([\\/)}\]-](?=[[:alpha]]))/g, // character
  everythingBeforeClosingCharAtEnd: /(.*)([)}\]]$)/g,
  openingChar: /[({[\]]/, // character
