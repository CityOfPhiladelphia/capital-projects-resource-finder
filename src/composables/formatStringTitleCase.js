...
export const formatStringTitleCase = (rawString) => {
  return expandContractions(rawString) // expand contractions
    .replace(regexDictionary.character.slashDashOrOpening, " $1") // insert space before '\', '/', -, '[', '{', '(' if character is preceded by a letter
    .replace(regexDictionary.character.slashDashOrClosing, "$1 ") // insert space after '\', '/', -, ']', '}', ')' if character is followed by a letter
