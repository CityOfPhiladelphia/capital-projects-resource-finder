...
export const expandContractions = (rawString) => {
  ...
  rawString = rawString.replace(/(?<=\W|\b)[Oo][Aa][Cc](\.(?=[ ])|(?=\W|\b))/g, 'older adult center') // oac to older adult center
  ...
  rawString = rawString.replace(/(?<=\W|\b)[Aa][Vv][Ee](\.(?=[ ])|(?=\W|\b))/g, 'avenue') // st to street
